...
const QUESTIONS = [
  ...
  {
    id: "moments", q: "What should I drop everything to photograph?", sub: "I'll keep my eyes open.",
    opts: ["Spontaneous tears", "Pure, unplanned laughter", "A quiet moment between you two", "Your favorite people, just being"],
  },
  {
    id: "size", q: "How many people are celebrating with you?", sub: "Big love, small gathering, or somewhere in between.",
  ...
  {
    id: "frame", q: "If you could only frame one photo from your wedding, what would it be?", sub: "The one that says everything.",
    opts: [
    ...
    >
      <motion.div variants={containerVariants} initial="hidden" animate="visible">
        <motion.p variants={itemVariants} style={{
          ...
          color: SADDLE, opacity: 0.7, marginBottom: "0.6rem",
        }}>
          {q.sub}
        </motion.p>
        <motion.h2 variants={itemVariants} style={{
          fontFamily: "'Libre Baskerville', serif", fontStyle: "italic",
          fontSize: "clamp(1.15rem, 3.5vw, 1.5rem)", fontWeight: 400,
          color: FIREBRICK, lineHeight: 1.35, marginBottom: "1.8rem",
        }}>
          {q.q}
        </motion.h2>
        <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem" }}>
          {q.opts.map((opt, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              style={{
                background: "transparent", border: `1.5px solid ${FIREBRICK}`,
                color: FIREBRICK, padding: "0.72rem 1.3rem",
                fontFamily: "'Manrope', sans-serif", fontSize: "0.93rem",
                cursor: "pointer", textAlign: "left", borderRadius: "999px",
                display: "flex", alignItems: "center", gap: "0.8rem",
                outline: "none", WebkitTapHighlightColor: "transparent",
                transition: "background 0.15s, color 0.15s",
              }}
            >
              <span className="q-btn-num" style={{ fontFamily: "monospace", fontSize: "0.65rem", color: KHAKI, minWidth: 18 }}>
                0{i + 1}
              </span>
              {opt}
            </motion.button>
...
function ContactForm({ onSubmit, answers }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await emailjs.send(EMAILJS_SERVICE, EMAILJS_TEMPLATE, {
        name: form.name, email: form.email, date: form.date,
  ...
  const inputBase = {
    width: "100%", background: "rgba(142,29,31,0.04)",
    border: "1px solid rgba(142,29,31,0.25)", color: FIREBRICK,
    fontFamily: "'Manrope', sans-serif", fontSize: "0.94rem",
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -48 }} transition={slideTransition}
      style={{ width: "100%", maxWidth: 420 }}
    >
      <motion.div variants={containerVariants} initial="hidden" animate="visible">
        <motion.p variants={itemVariants} style={{
          fontFamily: "'Manrope', sans-serif", fontSize: "0.72rem",
          letterSpacing: "0.2em", textTransform: "uppercase",
          color: SADDLE, opacity: 0.7, marginBottom: "0.4rem",
        }}>Almost there</motion.p>
        <motion.h2 variants={itemVariants} style={{
          fontFamily: "'Libre Baskerville', serif", fontStyle: "italic",
          fontSize: "1.4rem", fontWeight: 400, color: FIREBRICK, marginBottom: "0.3rem",
        }}>Tell me who you are.</motion.h2>
        <motion.p variants={itemVariants} style={{
          fontFamily: "'Manrope', sans-serif", fontSize: "0.88rem",
          color: SADDLE, marginBottom: "1.4rem", opacity: 0.75,
        }}>And when the story begins.</motion.p>

        {fields.map((field) => (
          <motion.div key={field.k} variants={itemVariants} style={{ marginBottom: "0.75rem", textAlign: "left" }}>
            <label style={{
              display: "block", color: SADDLE,
              fontFamily: "'Manrope', sans-serif", fontSize: "0.75rem",
              marginBottom: "0.22rem", letterSpacing: "0.05em",
            }}>{field.label}</label>
            {field.multiline ? (
              <textarea
                placeholder={field.ph} value={form[field.k]}
                onChange={e => setForm(p => ({ ...p, [field.k]: e.target.value }))}
                rows={3}
                style={{ ...inputBase, borderRadius: "16px", padding: "0.56rem 1rem", resize: "none" }}
                onFocus={e => e.target.style.borderColor = FIREBRICK}
                onBlur={e => e.target.style.borderColor = "rgba(142,29,31,0.25)"}
              />
            ) : (
              <input
                type="text" placeholder={field.ph} value={form[field.k]}
                onChange={e => setForm(p => ({ ...p, [field.k]: e.target.value }))}
                style={{ ...inputBase, borderRadius: "999px", padding: "0.56rem 1rem" }}
                onFocus={e => e.target.style.borderColor = FIREBRICK}
                onBlur={e => e.target.style.borderColor = "rgba(142,29,31,0.25)"}
              />
            )}
        ...
        <motion.button
          ...
          style={{
            ...
            border: "none", color: BISQUE, padding: "0.82rem 1.5rem",
        ...
        >
          {sending ? "Sending..." : "Begin the story"}
        </motion.button>
      </motion.div>
    </motion.div>
  );
  ...
  return (
    ...
    >
      <p style={{
        fontFamily: "'Manrope', sans-serif", fontSize: "0.72rem",
        letterSpacing: "0.2em", textTransform: "uppercase",
        color: SADDLE, opacity: 0.7, marginBottom: "0.8rem",
      }}>You're in good hands</p>
      <h2 style={{
        fontFamily: "'Libre Baskerville', serif", fontStyle: "italic",
        fontSize: "1.7rem", fontWeight: 400, color: FIREBRICK,
        marginBottom: "1rem", lineHeight: 1.3,
      }}>Can't wait to meet you.</h2>
      <p style={{
        fontFamily: "'Manrope', sans-serif", fontSize: "0.95rem",
        color: SADDLE, lineHeight: 1.8, marginBottom: "2rem", opacity: 0.8,
      }}>
      ...
      <button
        onClick={onClose}
        style={{
          background: "none", border: `1.5px solid ${FIREBRICK}`,
          color: FIREBRICK, padding: "0.7rem 2rem",
          fontFamily: "'Manrope', sans-serif", fontSize: "0.75rem",
          fontWeight: 500, cursor: "pointer", borderRadius: "999px",
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          background: "none", border: `1.5px solid ${FIREBRICK}`,
          color: FIREBRICK, width: 34, height: 34, borderRadius: "999px",
...
export default function SanaaPhotography() {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700&family=Libre+Baskerville:ital@0;1&display=swap');
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input::placeholder, textarea::placeholder { color: rgba(100,64,40,0.35); }
