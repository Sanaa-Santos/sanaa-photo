...
// ── PRICING CARD ──────────────────────────────────────────────────────────────
function PricingCard({ onOpenQuestionnaire }) {
  return (
    <Shell>
      <div style={{ padding:"2.5rem 1.5rem 3rem" }}>
        <FadeIn>
          <div style={{ border:`1.5px solid ${FIREBRICK}`, borderRadius:8,
            ...
            <p style={{ fontFamily:"'Manrope', sans-serif", fontSize:"0.62rem",
              letterSpacing:"0.22em", textTransform:"uppercase",
              color:FIREBRICK, opacity:0.85, marginBottom:"0.75rem" }}>
              Full Wedding Day
            </p>
            <p style={{ fontFamily:"'Libre Baskerville', serif",
              fontSize:"clamp(2.6rem, 11vw, 3.6rem)",
              fontWeight:400, color:FIREBRICK, lineHeight:1, marginBottom:"1.75rem" }}>
              $3,300
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:"0.85rem",
              textAlign:"left", marginBottom:"2rem" }}>
              {INCLUDED.map(item => (
                <p key={item} style={{ fontFamily:"'Manrope', sans-serif", fontSize:"0.85rem",
                  color:SADDLE, display:"flex", alignItems:"flex-start", gap:"0.6rem", margin:0 }}>
                  <span style={{ marginTop:"0.35rem" }}><Diamond color={FIREBRICK} size={7}/></span>
                  {item}
                </p>
              ))}
            </div>
...
// ── FAQ ───────────────────────────────────────────────────────────────────────
// All questions start collapsed; clicking the +/- toggles that question only.
...
function FaqSection() {
  // null = all collapsed by default (per Kas: no answer shown until clicked)
  const [openIndex, setOpenIndex] = useState(null);

  return (
    <Shell>
      <div style={{ padding:"1rem 1.5rem 3.5rem" }}>
        <FadeIn>
...
export default function InvestmentPage({ onOpenQuestionnaire }) {
  return (
    <>
      ...
      <main>
        <InvestmentHero/>
        <PricingCard onOpenQuestionnaire={onOpenQuestionnaire}/>
        <FaqSection/>
      </main>
